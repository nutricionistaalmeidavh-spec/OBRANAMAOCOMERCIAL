...
try{
  ...
  Invoke-Gate 'deploy-script-syntax' { $tokens=$null;$errors=$null;[System.Management.Automation.Language.Parser]::ParseFile((Join-Path $repoRoot 'scripts\deploy-catalog-production.ps1'),[ref]$tokens,[ref]$errors)|Out-Null;if($errors.Count -gt 0){$errors|ForEach-Object{Write-Error $_.Message};exit 1}else{Write-Host 'deploy-catalog-production.ps1 syntax OK'} }
