...
export const EDUCATION_SESSION_TTL_MS = 24 * 60 * 60 * 1000;
...
async function issue(id: string) { const t = crypto.randomUUID().replace(/-/g, '') + crypto.randomUUID().replace(/-/g, ''), stamp = now(); await db.add(sessionTable(t), [{ participantId: id, createdAt: stamp, expiresAt: new Date(Date.now() + EDUCATION_SESSION_TTL_MS).toISOString() }]); return t; }
async function participant(t: string) { if (t.length < 40)
    return null; const table=sessionTable(t),s = (await db.list<EduSession>(table, { limit: 1 })).items[0]; if (!s)
    return null; if(s.expiresAt < now()){await db.delete(table,[s.id]);return null} const [p] = await db.get<EduParticipant>('edu_participants', [s.participantId]); return p?.status === 'active' ? { ...p, id: s.participantId } : null; }
...
export function sameEducationTenant(actor:EduParticipant,target:EduParticipant){
    if(effectiveRole(actor)==='superadmin')return true;
    const companyId=String(actor.companyId||'').trim();
    return !!companyId&&companyId===String(target.companyId||'').trim();
}
...
export async function educationPhoneAccessState(inputPhone: string) {
    ...
    return { exists: true, needsPasswordSetup: p.mustChangePassword === true, participant: { id:p.id,name:p.name,companyName:p.companyName||null } };
}
...
'POST /api/edu/admin/overview': [async (c) => { const actor = await participant(String(rec(c.body).token || '')); if (!actor)
            ...
            return error('Revisão inválida.', 400); const items = (await db.list<EduParticipant>('edu_participants', { limit: 200 })).items, target = items.find(x => sameEducationTenant(actor,x) && (x.pendingReviews || []).some(r => r.id === reviewId)); if (!target)
            ...
            return error('Participante não encontrado.', 404); if(!sameEducationTenant(actor,target))return error('Participante fora da sua empresa.',403); const targetRole = effectiveRole(target), decision = educationRoleChangeDecision({ actorRole, targetRole, nextRole, isSelf: targetId === actor.id }); if (decision === 'admin-boundary')
