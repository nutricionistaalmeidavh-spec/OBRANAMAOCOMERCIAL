...
import { canApplyProviderTransition, normalizeAsaasEvent, validatePaidOrder, type FinancialStatus } from './billing-policy'
...
type Order={id:string;user_id:string;user_email:string;company_id:string|null;requested_company_name:string;plan_version_id:string;amount_cents:number;currency:string;financial_status:FinancialStatus;idempotency_key:string;provider:string;provider_customer_id:string|null;provider_payment_id:string|null;provider_subscription_id:string|null;provider_status:string|null;checkout_url:string|null;license_id:string|null;reconciliation_required:number;created_at:string;updated_at:string}
type Subscription={id:string;company_id:string|null;user_id:string;user_email:string;plan_version_id:string;provider:string;provider_customer_id:string|null;provider_subscription_id:string|null;financial_status:FinancialStatus;current_period_end:string|null;license_id:string|null;created_at:string;updated_at:string}
...
export async function checkoutForUser(env:BillingEnv,user:AuthUser,input:{planCode:string;companyName:string;idempotencyKey:string}){
...
async function activateOrder(env:BillingEnv,order:Order,plan:Plan,eventId:string,data:ReturnType<typeof providerData>){
  const valid=validatePaidOrder({orderAmountCents:order.amount_cents,providerAmountCents:data.amountCents,providerPaymentId:data.paymentId,expectedPaymentId:String(order.provider_payment_id||''),externalReference:data.externalReference,orderId:order.id});if(!valid.ok)throw new Error(`Pagamento rejeitado: ${valid.reason}`)
...
export async function processAsaasWebhookPayload(env:BillingEnv,payload:ProviderPayload){
  ...
  try{
    ...
    let order=data.externalReference?await orderById(env,data.externalReference):null;if(!order&&data.paymentId)order=await orderByPayment(env,data.paymentId)
    const subscription=data.subscriptionId?await subscriptionByProvider(env,data.subscriptionId):order?.provider_subscription_id?await subscriptionByProvider(env,order.provider_subscription_id):null
    if(next==='paid'){
      if(order&&order.financial_status!=='paid'){const plan=await planById(env,order.plan_version_id);if(!plan)throw new Error('Plano da ordem não encontrado.');if(!canApplyProviderTransition(order.financial_status,'paid'))throw new Error('Transição financeira inválida.');await activateOrder(env,order,plan,eventId,data)}
...
export async function billingStatus(env:BillingEnv,userId:string){const orders=await env.DB.prepare(`SELECT o.id,o.company_id,o.plan_version_id,o.amount_cents,o.currency,o.financial_status,o.checkout_url,o.license_id,o.reconciliation_required,o.created_at,o.updated_at,p.plan_code,p.name AS plan_name,p.interval_code FROM billing_orders o JOIN billing_plan_versions p ON p.id=o.plan_version_id WHERE o.user_id=? ORDER BY o.created_at DESC LIMIT 20`).bind(userId).all();const subscriptions=await env.DB.prepare(`SELECT s.id,s.company_id,s.plan_version_id,s.financial_status,s.current_period_end,s.license_id,s.created_at,s.updated_at,p.plan_code,p.name AS plan_name,p.price_cents,p.currency,p.interval_code FROM billing_subscriptions s JOIN billing_plan_versions p ON p.id=s.plan_version_id WHERE s.user_id=? ORDER BY s.updated_at DESC LIMIT 20`).bind(userId).all();return{orders:orders.results||[],subscriptions:subscriptions.results||[]}}
