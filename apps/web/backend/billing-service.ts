...
export async function billingStatus(env:BillingEnv,userId:string){const orders=await env.DB.prepare('SELECT id,company_id,plan_version_id,amount_cents,currency,financial_status,checkout_url,license_id,reconciliation_required,created_at,updated_at FROM billing_orders WHERE user_id=? ORDER BY created_at DESC LIMIT 20').bind(userId).all();const subscriptions=await env.DB.prepare('SELECT id,company_id,plan_version_id,financial_status,current_period_end,license_id,created_at,updated_at FROM billing_subscriptions WHERE user_id=? ORDER BY updated_at DESC LIMIT 20').bind(userId).all();return{orders:orders.results||[],subscriptions:subscriptions.results||[]}}
