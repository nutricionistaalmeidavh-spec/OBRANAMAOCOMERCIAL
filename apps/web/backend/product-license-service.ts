...
type OverviewAccount={email:string;status:string;source?:string};
type OverviewLicense={email:string;plan_code:string;status:string;expires_at:string|null;source?:string;updated_at?:string};
export type DeboraAdminClient={email:string;planCode:string;status:'active'|'freemium'|'revoked'|'expired';expiresAt:string|null;source:string;expiring:boolean};
...
export function buildDeboraAdminClients(accounts:OverviewAccount[],licenses:OverviewLicense[],now=new Date().toISOString()):DeboraAdminClient[]{
  ...
  return [...byEmail.entries()].map(([email,account])=>{
    ...
    const latest=active||own[0]||null;
    const latestExpiry=latest?.expires_at?Date.parse(latest.expires_at):null;
    const status:DeboraAdminClient['status']=active?'active':latest?.status==='revoked'?'revoked':latest?.expires_at&&typeof latestExpiry==='number'&&Number.isFinite(latestExpiry)&&latestExpiry<=current?'expired':'freemium';
    const expiring=Boolean(active?.expires_at&&typeof latestExpiry==='number'&&Number.isFinite(latestExpiry)&&latestExpiry>current&&latestExpiry<=expiringLimit);
    return{email,planCode:active?.plan_code||latest?.plan_code||'freemium',status,expiresAt:latest?.expires_at||null,source:latest?.source||account.source||'cloudflare_d1',expiring};
...
export function summarizeDeboraLicenseOverview(accounts:OverviewAccount[],licenses:OverviewLicense[],now=new Date().toISOString()){
  const clients=buildDeboraAdminClients(accounts,licenses,now);
  const pro=clients.filter(client=>client.status==='active').length;
  const expiring=clients.filter(client=>client.expiring).length;
  const revoked=clients.filter(client=>client.status==='revoked').length;
  return{clients:clients.length,pro,freemium:clients.length-pro,expiring,revoked};
}
