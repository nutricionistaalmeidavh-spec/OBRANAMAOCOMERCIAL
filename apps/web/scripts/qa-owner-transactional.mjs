...
try{
  ...
  for(const viewport of viewports){
    ...
    await form.locator('button[type="submit"]').click();await page.locator('#createResult').filter({hasText:'primeiro acesso é reconhecido por esse e-mail'}).waitFor({state:'visible'});await shot('03-company-created-by-email');
    await page.locator('[data-owner-view="clients"]').click();await page.locator('[data-company="qa-company-1"]').click();const licenseForm=page.locator('#licenseForm');await licenseForm.waitFor({state:'visible'});await shot('04-company-license-detail');
