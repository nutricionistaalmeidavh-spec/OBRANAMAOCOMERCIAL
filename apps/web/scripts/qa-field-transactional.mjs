...
try{
  ...
  for(const viewport of viewports){
    ...
    await page.getByText('Ana QA').first().waitFor({state:'visible'});await shot('01-day-remote-state');
    ...
    await page.locator('.plan-card').filter({hasText:'AF/AQ'}).locator('.plan-start').click();await page.locator('#createSession').waitFor({state:'visible'});const crewChecks=page.locator('#sessionCrew input');if(await crewChecks.count()>=2){await crewChecks.nth(0).check();await crewChecks.nth(1).check()}await page.locator('#sstart').fill('08:00');await page.locator('#createSession').click();await page.locator('.session-card').waitFor({state:'visible'});await shot('03-session-running');
    await page.locator('.session-card .session-pause').click();await page.locator('.session-card .session-resume').waitFor({state:'visible'});await page.locator('.session-card .session-resume').click();await page.locator('.session-card .session-finish').waitFor({state:'visible'});await page.locator('.session-card .session-finish').click();await page.locator('#send').fill('09:00');await page.locator('#sresult').selectOption('completed');await page.locator('#snote').fill('Execução concluída QA');await page.locator('#finishSession').click();await page.locator('.session-card').filter({hasText:'Sessão concluída'}).waitFor({state:'visible'});await shot('04-session-finished');
    ...
    await page.locator('[data-screen="today"]').first().click();await page.locator('[data-screen="settings"]').first().click();await page.locator('#defaultStart').fill('08:00');await page.locator('#saveDefaultStart').click();await page.locator('#openUsersBtn').waitFor({state:'visible'});await page.locator('#openUsersBtn').click();await page.locator('#memberEmail').fill('encarregado.qa@example.test');await page.locator('#memberRole').selectOption('foreman');await page.locator('#saveMemberBtn').click();await page.getByText('encarregado.qa@example.test').waitFor({state:'visible'});await shot('08-user-created');await page.locator('#sheet .close').click();
