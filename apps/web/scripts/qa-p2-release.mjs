...
if(Number(ownerTransactional.licenseSuspensionsPerformed)!==0)invariantErrors.push('transactional owner QA suspended a license');
...
if(fieldTransactional.status!=='passed')invariantErrors.push('field transactional report not passed');
...
if(Number(fieldTransactional.licenseMutationsPerformed)!==0)invariantErrors.push('field transactional QA touched commercial licensing');
...
console.log('ARTISYS_QA_P2_BUNDLE='+outDir);console.log(JSON.stringify({status:summary.status,gate:allowed,blockers:summary.gate.blockers,coverage:{basis:coverage.basis,covered:coverage.covered,discovered:coverage.discovered,uncoveredCritical:coverage.uncoveredCritical},ownerViewports:actualViewports,fieldViewports},null,2));if(!allowed)process.exitCode=1;
