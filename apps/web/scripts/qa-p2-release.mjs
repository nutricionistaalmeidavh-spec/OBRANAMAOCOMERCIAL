...
const checks=[];
checks.push(run('admin:parity',npmCommand,['run','qa:admin-parity']));
...
console.log('ARTISYS_QA_P2_BUNDLE='+outDir);console.log(JSON.stringify({status:summary.status,gate:allowed,blockers:summary.gate.blockers,coverage:{basis:coverage.basis,covered:coverage.covered,discovered:coverage.discovered,uncoveredCritical:coverage.uncoveredCritical},adminParity:statusByCheck.get('admin:parity'),ownerViewports:actualViewports,fieldViewports,governanceViewports,desktopRendererPassed:desktopReportPassed},null,2));if(!allowed)process.exitCode=1;