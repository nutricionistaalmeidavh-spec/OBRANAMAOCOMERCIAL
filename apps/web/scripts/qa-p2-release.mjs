...
if(ownerTransactional.status!=='passed')invariantErrors.push('owner transactional report not passed');
...
if(Number(ownerTransactional.licenseSuspensionsPerformed)!==0)invariantErrors.push('transactional QA suspended a license');
const expectedViewports=['desktop','tablet','mobile'];const actualViewports=Array.isArray(ownerTransactional.viewports)?ownerTransactional.viewports:[];for(const viewport of expectedViewports)if(!actualViewports.includes(viewport))invariantErrors.push(`missing ${viewport} browser coverage`);
...
console.log('ARTISYS_QA_P2_BUNDLE='+outDir);console.log(JSON.stringify({status:summary.status,gate:allowed,blockers:summary.gate.blockers,coverage:{basis:coverage.basis,covered:coverage.covered,discovered:coverage.discovered,uncoveredCritical:coverage.uncoveredCritical},viewports:actualViewports},null,2));if(!allowed)process.exitCode=1;
