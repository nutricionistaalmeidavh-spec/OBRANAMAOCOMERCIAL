...
for (const product of catalog.products) {
  ...
  assert.ok(!slugs.has(product.slug), `duplicate slug: ${product.slug}`); slugs.add(product.slug);
  assert.equal(typeof product.name, 'string'); assert.equal(typeof product.category, 'string'); assert.ok(allowedTypes.has(product.type)); assert.equal(typeof product.status, 'string'); assert.equal(typeof product.priceLabel, 'string'); assert.equal(typeof product.summary, 'string'); assert.equal(typeof product.audience, 'string');
  ...
  assert.equal(typeof product.featured, 'boolean'); assert.equal(typeof product.ctaLabel, 'string'); assert.ok(product.ctaHref.startsWith('/') || product.ctaHref.startsWith('https://')); assert.equal(typeof product.seo?.title, 'string'); assert.equal(typeof product.seo?.description, 'string');
  ...
  assert.ok(page.includes(product.name), `${product.slug} product name missing`); assert.ok(page.includes(`<link rel="canonical" href="${canonical}">`), `${product.slug} canonical missing`); assert.match(page, /<meta property="og:title"/i); assert.match(page, /<meta name="twitter:card"/i); assert.match(page, /<script type="application\/ld\+json">/i); assert.match(page, /class="product-hero"/i); assert.match(page, /class="feature-grid"/i); assert.match(page, /class="faq-list"/i); assert.match(page, /data-product-cta/i); assert.doesNotMatch(page, /\/src\/|cloudflare-client|\/api\/|portal\.ts|owner\.ts|field\.ts|type="module"/i);
}
...
assert.match(js, /fetch\(['"]\.\/products\.json['"]\)/i); assert.match(js, /`\.\/\$\{product\.slug\}\//i, 'catalog cards must link to individual product pages'); assert.doesNotMatch(js, /cloudflare-client|\/api\/|portal\.ts|owner\.ts|field\.ts/i); assert.ok(css.length > 3000); new Function(js);
