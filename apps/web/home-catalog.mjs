...
export function renderHomeCatalogSection() {
  const cards = catalog.products.map((product) => `<article class="artisys-product-card"><div class="artisys-product-meta"><span>${escapeHtml(product.category)}</span><span>${escapeHtml(typeLabel(product.type))}</span></div><h3>${escapeHtml(product.name)}</h3><p>${escapeHtml(product.summary)}</p><div class="artisys-product-card-bottom"><strong>${escapeHtml(product.priceLabel)}</strong><a href="/sistemas/${escapeHtml(product.slug)}/">Ver sistema <span aria-hidden="true">↗</span></a></div></article>`).join('');
