...
function deboraView(){return `<section class="owner-product-banner debora"><div><span class="owner-eyebrow">PRODUTO</span><h2>Débora Lactação</h2><p>Controle comercial da assinatura sem misturar com o provisionamento empresarial do Obra na Mão.</p></div><div class="owner-banner-stats"><strong>${deboraOverview.pro}</strong><span>Pro ativo(s)</span></div></section><section class="owner-section"><div class="owner-section-head"><div><span class="owner-eyebrow">CARTEIRA</span><h2>Clientes e status</h2><p>Contagens reais do D1 central da Artisys.</p></div></div><div class="owner-metrics owner-debora-metrics">${deboraMetric('Clientes',deboraOverview.clients,'clients','Contas comerciais')}${deboraMetric('Pro ativo',deboraOverview.pro,'pro','Acesso completo')}${deboraMetric('Freemium',deboraOverview.freemium,'freemium','Limite gratuito')}${deboraMetric('Expirando',deboraOverview.expiring,'expiring','Próximos 30 dias')}${deboraMetric('Revogadas',deboraOverview.revoked,'revoked','Sem Pro ativo')}</div></section>${deboraLicenseCard()}`}
...
async function refreshDeboraOverview(){const {data}=await api.get<{overview:DeboraOverview}>('/api/owner/debora-overview');deboraOverview=data.overview||emptyDeboraOverview()}
