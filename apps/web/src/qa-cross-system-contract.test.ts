...
describe('Central ↔ Loja Online QA cross-system runner',()=>{
  ...
  it('never serializes the shared secret into the report',async()=>{
    ...
    const fetchImpl=async(input:RequestInfo|URL,init?:RequestInit)=>{
      const url=String(input);
      if(url.includes('/api/internal/artisys/loja-online/companies')&&init?.method==='POST'){
        expect(new Headers(init.headers).get('x-artisys-license-secret')).toBe(secret);
        return Response.json({company,license,admin:{id:'usr_qa',email:'qa@example.test'},temporaryPassword:'temp-password'},{status:201});
      }
      ...
      if(url.endsWith(`/companies/${company.id}`)) return Response.json({company,license});
    ...
    expect(JSON.stringify(report)).not.toContain('temp-password');
  });
});
