...
function guideHtml(context:GuideContext,compact=false){const[title,text]=guideCopy(context);return'<aside class="edu-mh-guide '+(compact?'edu-mh-guide--compact':'')+'" data-guide-context="'+context+'"><div class="edu-mh-avatar" aria-hidden="true"><img class="edu-mh-frame" src="/artisys-icon.svg" alt=""></div><div class="edu-mh-bubble"><small>GUIA ARTISYS</small><strong>'+title+'</strong><p>'+text+'</p></div></aside>'}
...
function upgradeLegacyGuide(root:ParentNode=document){root.querySelectorAll<HTMLElement>('.edu-mh-mascot').forEach(node=>{node.className='edu-mh-avatar';node.innerHTML='<img class="edu-mh-frame" src="/artisys-icon.svg" alt="">'})}
function addPortalLink(root:ParentNode=document){const header=root.querySelector('.edu-top');if(header&&!header.querySelector('.edu-portal-link'))header.insertAdjacentHTML('beforeend','<a class="edu-portal-link" href="./index.html#portal" aria-label="Voltar ao Portal Artisys">Portal Artisys</a>')}
...
export function createUniversityShell(input:{participant:()=>ShellParticipant|null;onNavigate:(id:string)=>void}){
  ...
  const render=(title:string,html:string,active='inicio',guide?:GuideContext)=>{
    ...
    document.body.innerHTML='<div class="edu-app"><aside class="edu-side"><div class="edu-brand edu-brand--artisys"><img src="/artisys-logo.svg" alt="Artisys"></div><small class="edu-version">v'+APP_VERSION+'</small><nav aria-label="Navegação principal">'+nav.map(item=>'<button data-nav="'+item.id+'" class="'+(active===item.id?'active':'')+'" '+(active===item.id?'aria-current="page"':'')+'>'+item.label+'</button>').join('')+'</nav></aside><main><header class="edu-top"><strong>'+title+'</strong><span>'+(participant?.name||'')+'<small>'+(participant?.jobRole||'')+'</small></span></header><section class="edu-content">'+pageContent(html,pageGuide)+'</section></main></div><div id="eduToast" class="edu-toast" role="status" aria-live="polite"></div>';
