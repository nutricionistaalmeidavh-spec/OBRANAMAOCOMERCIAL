...
function ensureOverview(){
  ...
  card.innerHTML=`<div class="owner-product-icon owner-product-icon-loja">LO</div><div><span class="owner-product-label">VAREJO / SaaS</span><h3>Loja Online</h3><p>${state.error?esc(state.error):`${state.overview.totalClients} loja(s) · ${state.overview.active} ativa(s).`}</p></div><button type="button" class="owner-primary" data-loja-open>Gerenciar</button>`;
