...
const adminUi = createUniversityAdminUi({token:tok,render:(title,html,nav)=>ui(title,html,nav),notify:say,errorMessage:message,canAccessAdmin:admin,canManageRoles,onUnauthorized:home,openTracks:trilhas});
...
async function lesson(id: string) {
    ...
    const renderQuestion = () => {
        ...
        const controls = item.kind === 'text' || item.kind === 'short-text'
            ? '<label for="eduAnswer">Sua resposta<input id="eduAnswer" class="edu-text-input" autocomplete="off"></label>'
            : '<fieldset class="edu-options"><legend>Escolha uma alternativa</legend>' + (item.options || []).map(option => '<label class="edu-option"><input name="edu-answer" type="radio" value="' + esc(option) + '"><span>' + esc(option) + '</span></label>').join('') + '</fieldset>';
        const visual = hasQuestionVisual(item.prompt) ? '<figure class="edu-question-visual" data-question-visual data-question-id="' + esc(item.meta?.id || '') + '" data-question-prompt="' + esc(item.prompt) + '"><span class="edu-question-visual-loading">Carregando imagem de apoio…</span></figure>' : '';
